...
THREEx.MinecraftControls = function (object3d, input) {
  ...
  this.update = function (delta, now) {
    ...
    if (this.lateralMove === "rotationY") {
      // lateral => rotation Y
      if (input.left) object3d.rotation.y += 0.1 * delta * Math.PI * 1;
      if (input.right) object3d.rotation.y -= 0.1 * delta * Math.PI * 1;
    } else if (this.lateralMove === "rotationNE") {
      // lateral => rotation Y
      object3d.rotation.y -= 0.1 * delta * Math.PI * 1;
      object3d.rotation.x -= 0.1 * delta * Math.PI * 1;
    } else if (this.lateralMove === "rotationNW") {
      // lateral => rotation Y
      object3d.rotation.y += 0.1 * delta * Math.PI * 2;
      object3d.rotation.x -= 0.1 * delta * Math.PI * 2;
    } else if (this.lateralMove === "rotationSE") {
      // lateral => rotation Y
      object3d.rotation.y -= 0.1 * delta * Math.PI * 2;
      object3d.rotation.x += 0.1 * delta * Math.PI * 2;
    } else if (this.lateralMove === "rotationSW") {
      // lateral => rotation Y
      object3d.rotation.y += 0.1 * delta * Math.PI * 2;
      object3d.rotation.x += 0.1 * delta * Math.PI * 2;
      ...
      console.assert(false, "this.lateralMove invalid: " + this.lateralMove);
